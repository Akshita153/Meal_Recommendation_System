...
export const addHealthDetails = async (req, res, next) => {
  ...
  const { age, height, gender, weight, activityLevel, totalCalories } =
    req.body;
  ...
  try {
    jwt.verify(token, jwtSecret, {}, async (err, userData) => {
      if (err) {
        ...
        const existingHealthData = await HealthData.findOne({
          user: userData.id,
        });
        ...
        if (existingHealthData) {
          ...
          existingHealthData.gender = gender;
  ...
  try {
    jwt.verify(token, jwtSecret, {}, async (err, userData) => {
      if (err) {
        ...
        const healthData = await HealthData.find({ user: userData.id });
