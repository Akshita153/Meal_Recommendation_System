...
import egg from "../../assets/chicken.gif";

import tree from "../../assets/egg.webp";
...
const ChickenMeal = () => {
    ...
    useEffect(() => {
        // Use useEffect to capture changes in the location state
        if (location.state && location.state.totalCalories) {
            setTotalCalories(location.state.totalCalories);
        }
    }, [location.state]);
    ...
    const mealgeneration = () => {
        ...
        switch (meal) {
            ...
            case "2":
                ...
                const random_dish =
                    main_dishes[Math.floor(Math.random() * main_dishes.length)];
                setgeneratedLunch(random_dish);

                const roti_type = ["Chapati", "Bhakri"];

                setRotiType(roti_type[Math.floor(Math.random() * roti_type.length)]);

                const rice_type = ["Daal & Rice", "Veg Pulao"];
            ...
            case "3":
                ...
                SetGeneratedDinner(
                    dinner_dishes[Math.floor(Math.random() * dinner_dishes.length)]
                );

                const roti__type = ["Chapati", "Bhakri"];

                setRotiType(roti__type[Math.floor(Math.random() * roti__type.length)]);

                const rice__type = ["Daal & Rice", "Veg Pulao"];
    ...
    return (
        <div>
        <Navbar />
        <div style={{ background: "linear-gradient(to bottom, #f72d2d, white)" }}>
                ...
                <Grid item xs={3} sx={{ display: "flex", justifyContent: "center" }}>
                    <Box>
                        <img
                            src={tree}
                            alt="Your Gif"
                            style={{ width: "70%", height: "auto", marginTop: "1rem" }}
                        />
                    </Box>
                </Grid>
            </Grid>
